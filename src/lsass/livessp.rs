...
use crate::lsass::types::{
    Arch, LiveSspCredential, is_valid_user_ptr, read_ptr, read_ustring, walk_list,
};
...
pub fn extract_livessp_credentials_arch(
    ...
    let text_base = livessp_base + u64::from(text.virtual_address);
    ...
    let list_addr = match arch {
        ...
        Arch::X86 => {
            ...
            let data_base = livessp_base + u64::from(ds.virtual_address);
    ...
    log::info!("LiveSSP list at 0x{list_addr:x}");
    ...
    walk_list(vmem, list_addr, arch, |current| {
        ...
        if is_valid_user_ptr(supp_ptr, arch) {
            let username =
                read_ustring(vmem, supp_ptr + offsets.supp_username, arch).unwrap_or_default();
            let domain =
                read_ustring(vmem, supp_ptr + offsets.supp_domain, arch).unwrap_or_default();

            if !username.is_empty() {
                let password = crate::lsass::crypto::decrypt_unicode_string_password_arch(
                    vmem,
                    supp_ptr + offsets.supp_password,
                    keys,
                    arch,
                );
                log::debug!(
