...
use crate::lsass::types::{
    Arch, WdigestCredential, is_valid_user_ptr, read_ptr, read_ustring, scan_data_for_list_head,
    walk_list,
};
...
pub fn extract_wdigest_credentials_arch(
    ...
    let (pattern_list, pattern_label) = match arch {
        ...
        Arch::X86 => (
            patterns::WDIGEST_LOGON_SESSION_PATTERNS_X86,
            "wdigest_l_LogSessList_x86",
        ),
    };
    ...
    log::info!("WDigest l_LogSessList at 0x{list_addr:x} (arch={arch:?})");
...
fn find_wdigest_list_in_data(
    ...
    scan_data_for_list_head(
        ...
        |flink, list_addr| {
            ...
            log::debug!(
                "Found l_LogSessList candidate at 0x{list_addr:x}: flink=0x{flink:x} LUID=0x{luid:x} user='{username}'"
            );
            true
        },
    )
